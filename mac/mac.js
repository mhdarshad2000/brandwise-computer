...
const brandUrl = "https://www.service-center-locator.com/mac/mac-service-center.htm"
    ...
    return new Promise(async (resolve) => {
        try {
            ...
            $(postDiv).find(" div:nth-child(9),div:nth-child(10)").children("ul").map((i, state) => {
                mac[i] = {}
                mac[i]['state'] = ($(state).children("strong").text())
                mac[i]['states'] = []
                $(state).children("li").each(async (j, city) => {
                    mac[i]['states'][j] = {}
                    mac[i]['states'][j]['name'] = $(city).text()
                    const link = $(city).children("a").attr("href").replace("../", baseUrl)
                    mac[i]['states'][j]['link'] = link

                    mac[i]['states'][j]['city'] = await detailsPage(link, $(city).text())
                })

            })
            setTimeout(() => {
                const brand = JSON.stringify(mac)
                fs.writeFileSync("./mac/mac.json", brand)
