...
const brandUrl = "https://www.service-center-locator.com/microsoft/microsoft-service-center.htm"
    ...
    return new Promise(async (resolve) => {
        try {
            ...
            $(postDiv).find(" div:nth-child(11),div:nth-child(12)").children("ul").map((i, state) => {
                microsoft[i] = {}
                microsoft[i]['state'] = ($(state).children("strong").text())
                microsoft[i]['states'] = []
                $(state).children("li").each(async (j, city) => {
                    microsoft[i]['states'][j] = {}
                    microsoft[i]['states'][j]['name'] = $(city).text()
                    const link = $(city).children("a").attr("href").replace("../", baseUrl)
                    microsoft[i]['states'][j]['link'] = link

                    microsoft[i]['states'][j]['city'] = await detailsPage(link, $(city).text())
                })

            })
            setTimeout(() => {
                const brand = JSON.stringify(microsoft)
                fs.writeFileSync("./microsoft/microsoft.json", brand)
        ...
        try {
            ...
            if (tableDiv.text()) {
                ...
                $(postDiv).children("h2").each((i,serviceCenter)=>{
                    if(!$(serviceCenter).text().includes("Microsoft Service Centers in")){
                        arr[i-1]={}
