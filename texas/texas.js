    ...
    return new Promise(async (resolve) => {
        try {
            ...
            $(postDiv).find(" div:nth-child(11),div:nth-child(12)").children("ul").map((i, state) => {
                texas[i] = {}
                texas[i]['state'] = ($(state).children("strong").text())
                texas[i]['states'] = []
                $(state).children("li").each(async (j, city) => {
                    texas[i]['states'][j] = {}
                    texas[i]['states'][j]['name'] = $(city).text()
                    const link = $(city).children("a").attr("href").replace("../", baseUrl)
                    texas[i]['states'][j]['link'] = link

                    texas[i]['states'][j]['city'] = await detailsPage(link, $(city).text())
                })

            })
            setTimeout(() => {
                const brand = JSON.stringify(texas)
                fs.writeFileSync("./texas/texasInstruments.json", brand)
