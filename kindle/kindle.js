    ...
    return new Promise(async (resolve) => {
        try {
            ...
            $(postDiv).find(" div:nth-child(11),div:nth-child(12)").children("ul").map((i, state) => {
                kindle[i] = {}
                kindle[i]['state'] = ($(state).children("strong").text())
                kindle[i]['states'] = []
                $(state).children("li").each(async (j, city) => {
                    kindle[i]['states'][j] = {}
                    kindle[i]['states'][j]['name'] = $(city).text()
                    const link = $(city).children("a").attr("href").replace("../", baseUrl)
                    kindle[i]['states'][j]['link'] = link

                    kindle[i]['states'][j]['city'] = await detailsPage(link, $(city).text())
                })

            })
            setTimeout(() => {
                const brand = JSON.stringify(kindle)
                fs.writeFileSync("./kindle/kindle.json", brand)
